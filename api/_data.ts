...
export function getAiRecipes() {
  return aiGeneratedRecipes;
}
...
export function addShoppingItem(name: string, quantity: string = '1') {
  const newItem = {
...
export function updateShoppingItem(id: string, completed: boolean) {
...
export function removeShoppingItem(id: string) {
  ...
  return mockShoppingList.length < initialLength;
}
...
export function matchRecipes(ingredients: { name: string }[]) {
  const ingredientNames = ingredients.map((i) => i.name.toLowerCase());

  return mockRecipes.filter((recipe) => {
    const recipeIngredients = recipe.ingredients.map((i) => i.name.toLowerCase());
    return recipeIngredients.some((ri) =>
      ingredientNames.some((iname) => ri.includes(iname) || iname.includes(ri))
    );
  });
}
